...
!function($){
            ...
            $.ajax({
                ...
                success:function(data){
                    if(data.status==0){
                        ...
                        changeAddressData($('.J_addressItem').eq(0));
                    }else{
                        alert(data.msg);
                    }
    ...
    function createAddress(data,aid){
        ...
        html += '<a href="javascript:void(0);" data-id="1" class="modify J_addressModify">修改</a>';
        html += '</div></div>'; 

        var $newAddress = $(html).prependTo($('#J_addressList'));
        $newAddress.find('.J_addressItem').data('address_id',aid);
        $newAddress.find('.J_addressItem').data('consignee',data.uname);
        $newAddress.find('.J_addressItem').data('tel',data.phone);
        $newAddress.find('.J_addressItem').data('province_name',data.province);
        $newAddress.find('.J_addressItem').data('city_name',data.city);
        $newAddress.find('.J_addressItem').data('district_name',data.country);
        $newAddress.find('.J_addressItem').data('address',data.address);
    ...
    $('#J_addressList').on('click','.J_addressItem',function(){
        ...
        changeAddressData($(this));
        orderData.addressId = $(this).data('address_id');
    })
    function changeAddressData(obj){
        var data = {
            uname:obj.data('consignee'),
            phone:obj.data('tel'),
            province:obj.data('province_name'),
            city:obj.data('city_name'),
            country:obj.data('district_name'),
            address:obj.data('address')
        };
        $('#J_confirmAddress').html(data.uname+' '+data.phone+'<br>'+data.province+' '+data.city+' '+data.country+' '+data.address).removeClass('hide');
        orderData.addressId = obj.data('address_id');
    }
    $.ajaxSetup({
        headers: {
    ...
    $('#J_checkoutToPay').on('click',function(ev){
        ...
        if(!orderData.addressId){
            $('body').animate({scrollTop:$('.container').offset().top},250,function(){
                alert('请选择收货地址');
            });
            return ;
        }
        $.ajax({
            ...
            success: function(data){
                if(data.status ==0){
                    window.location.href = '/order/pay?id='+data.msg;
                }else{
                    alert(data.msg);
                }
            }
